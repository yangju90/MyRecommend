...
class MF(object):
    ...
    def update(self,alpha = 0.001,remeda=0.05):
        ...
        pE = np.dot(U,M.T)+self.overall_mean+b_U.repeat(self._m_num,axis=1)+b_M.repeat(self._u_num,axis=1).T
        return self.rmse(pE,self.data),pE
    ...
    def factorize(self,s,k=50,iter=100,alpha=0.001,remeda=0.05,descent=False,descent_rate=0.9,pp=False):
        '''
        Apply SGD for MF
        ...
        for i in range(iter):
            if not pp:
                cost,pE = self.update(alpha,remeda)
            else:
            ...
            if gap<0.000085:
                print("model has converged.",i+1,new)
                i1 = i
                break
            prvs = cost
            """
